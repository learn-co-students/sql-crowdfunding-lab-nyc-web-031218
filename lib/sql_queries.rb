...
"SELECT title, (SUM(amount) - funding_goal) AS 'total' FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY title HAVING total >= 0;"
end
...
"SELECT category, pledges.amount FROM projects INNER JOIN pledges ON projects.id = pledges.project_id WHERE projects.category = 'music';"
